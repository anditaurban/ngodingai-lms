...
export default function ProfilePage() {
   const { 
      user, loading, handleLogout, uploadPhoto, 
      isUploadingPhoto, setIsEditing 
   } = useProfileLogic();
   ...
   useEffect(() => {
      ...
      return () => window.removeEventListener('triggerEditProfile', handleTriggerEdit);
   }, [setIsEditing]);
   ...
   const handleRotateImage = async () => {
      ...
      try {
         ...
         if (currentPhotoUrl.startsWith('data:image')) {
             ...
             if (urlToFetch.startsWith('http') && urlToFetch.includes('dev.katib.cloud')) {
   ...
   if (!user) {
      return (
         <div className="flex flex-col items-center justify-center min-h-[60vh] text-center p-6">
            ...
            <button onClick={handleLogout} className="text-[#00BCD4] font-bold hover:underline">
               Login Kembali
            </button>
         </div>
      );
