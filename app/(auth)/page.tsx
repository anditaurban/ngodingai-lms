         ...
         <div className="w-full max-w-lg glass-panel rounded-2xl sm:rounded-4xl shadow-soft border border-white/50 dark:border-white/10 p-8 sm:p-10 flex flex-col gap-8 transition-all duration-300">
            ...
            <form onSubmit={(e) => e.preventDefault()} className="flex flex-col gap-6">
                ...
                <div className="flex flex-col gap-2">
                    <label className="text-sm font-bold uppercase tracking-wider mb-1 text-slate-700 dark:text-slate-300">WhatsApp Number</label>
                    <div className={`group flex w-full items-center rounded-xl border border-[#eaeaef] dark:border-gray-700 bg-white dark:bg-surface-dark transition-all duration-200 h-14 overflow-hidden relative ${loginState === 'otp' ? 'opacity-75 pointer-events-none bg-gray-50' : 'focus-within:border-indigo-500 focus-within:ring-4 focus-within:ring-indigo-500/10'}`}>
                ...
                {loginState === 'otp' && (
                  <div className="flex flex-col gap-4 animate-fade-in">
                    <label className="text-sm font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300">Enter 6-Digit Code</label>
                    ...
                    <p className="text-xs text-center text-[#666685] dark:text-gray-500">
                      Kode dikirim ke WhatsApp <span className="font-bold text-indigo-600">{phoneNumber}</span>
                    </p>
                  </div>
                )}
