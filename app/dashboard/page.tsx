...
export default function DashboardPage() {
  ...
  // --- LOGIC: MENGHUBUNGKAN DATA JSON KE TAMPILAN ---
  
  // 1. Hitung Kursus Aktif (Progress > 0 dan < 100)
  const activeCourses = coursesData.filter(c => c.progress > 0 && c.progress < 100);
  const activeCount = activeCourses.length;

  // 2. Logic Hero Course (Kursus Terakhir Diakses)
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
          ...
          {heroCourse && (
            <div className="bg-white dark:bg-slate-800 rounded-2xl p-1 border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col md:flex-row h-full">
                {/* PERBAIKAN 1: min-h-[200px] -> min-h-50 
                   (200px / 4 = 50 unit tailwind)
                */}
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-5">
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-3 items-center group cursor-pointer">
                {/* PERBAIKAN 2: min-w-[50px] -> min-w-12.5 
                   (50px / 4 = 12.5 unit tailwind)
                */}
              ...
              {/* Event 2 */}
              <div className="flex gap-3 items-center group cursor-pointer">
                {/* PERBAIKAN 3: min-w-[50px] -> min-w-12.5 */}
