...
export default function ArticleWorkspace({ title, setTitle, initialContent, onContentUpdate, onSave, onDelete }: ArticleWorkspaceProps) {
  ...
  // Ref untuk menghindari stale state (closure) saat auto-sync dipanggil dari dalam Tiptap onUpdate
  ...
  // ✨ ENGINE PROTOTYPE MAGIC: Auto-Sync Real-Time ke LocalStorage ✨
  const autoSyncToStorage = (newText: string, newTitle: string) => {
    try {
      if (typeof window === 'undefined') return;
      const urlParams = new URLSearchParams(window.location.search);
      const courseSlug = urlParams.get('course') || 'ngodingai';
      const chapterId = urlParams.get('chapter');
      
      if (!chapterId) return;

      const key = `db_content_${courseSlug}_${chapterId}`;
      const existingDataStr = localStorage.getItem(key);
      const existingData = existingDataStr ? JSON.parse(existingDataStr) : {};
      
      existingData.content = newText;
      existingData.title = newTitle;
      existingData.type = 'article';
      
      localStorage.setItem(key, JSON.stringify(existingData));
      
      // Tembakkan sinyal ke seluruh tab yang terbuka agar langsung refresh otomatis!
      window.dispatchEvent(new Event('storage'));
    } catch (e) {
      console.error("Gagal melakukan Auto-Sync", e);
    }
  };
  ...
  const editor = useEditor({
    ...
    onUpdate: ({ editor }) => {
      ...
      // Auto-Sync saat teks diketik!
      autoSyncToStorage(htmlContent, titleRef.current);
    },
    editorProps: {
      attributes: {
  ...
  // ✨ FIX: Paksa Tiptap untuk memuat materi baru ketika Instruktur berpindah bab ✨
  useEffect(() => {
    if (editor && initialContent !== editor.getHTML()) {
        let freshContent = initialContent;
        if (freshContent.includes("Mulai ketik materi")) freshContent = ""; 
      ...
      <div className="max-w-4xl w-full mx-auto px-6 md:px-16 py-12 pb-40 flex flex-col">
        ...
        <div className="flex items-center justify-between mb-8">
           <div className="flex items-center gap-2 text-[#00BCD4] font-bold text-sm tracking-wider uppercase bg-cyan-50 dark:bg-cyan-900/20 px-3 py-1.5 rounded-lg">
              ...
              {/* Indikator Auto-Sync Menyala */}
              <span className="ml-2 px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-600 text-[9px] flex items-center gap-1 animate-pulse"><span className="size-1.5 rounded-full bg-emerald-500"></span>Auto-Sync</span>
           </div>
        ...
        <div className="group relative">
          <textarea 
            ...
            onChange={(e) => {
               ...
               // Auto-Sync saat judul diubah!
               autoSyncToStorage(editor?.getHTML() || '', e.target.value);
            }} 
