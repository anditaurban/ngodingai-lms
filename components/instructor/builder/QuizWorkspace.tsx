...
export default function QuizWorkspace({ title, setTitle, initialQuestions, onSave, onDelete }: QuizWorkspaceProps) {
  ...
  return (
    <div className="max-w-4xl w-full mx-auto px-6 md:px-16 py-12 pb-40 flex flex-col gap-8 animate-fade-in">
      <div className="flex flex-col gap-4">
        <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800/50 pb-4">
           ...
           <div className="flex items-center gap-3">
              ...
              {/* ✨ MENGIRIM PAYLOAD QUESTIONS KE ONSAVE ✨ */}
              <button onClick={() => onSave({ questions })} className="flex items-center gap-2 px-5 py-2 bg-amber-500 hover:bg-amber-600 text-white rounded-xl text-xs font-bold shadow-lg shadow-amber-500/20 transition-all active:scale-95">
                 <span className="material-symbols-outlined text-[16px]">save</span> Simpan Perubahan
              </button>
           </div>
        </div>
        <textarea value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Judul Kuis..." className={`w-full bg-transparent text-4xl md:text-5xl lg:text-[56px] font-extrabold text-slate-900 dark:text-white placeholder:text-slate-200 dark:placeholder:text-slate-800 resize-none outline-none border-none focus:ring-0 overflow-hidden leading-[1.1] ${googleSansAlt.className}`} rows={2} />
      </div>
        ...
        {questions.map((q, qIndex) => (
          <div key={q.id} className="bg-white dark:bg-[#111111] p-6 md:p-8 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col gap-6 relative overflow-hidden group">
            ...
            <div className="flex justify-between items-start gap-4">
               <span className="bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 size-8 rounded-full flex items-center justify-center font-bold text-sm shrink-0 shadow-inner">{qIndex + 1}</span>
               ...
               <button onClick={() => removeQuestion(q.id)} className="text-slate-300 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 p-2 rounded-lg transition-colors"><span className="material-symbols-outlined text-[20px]">delete</span></button>
            </div>
          </div>
        ))}
      </div>
      <button onClick={addQuestion} className="w-full py-5 border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-2xl font-bold text-slate-500 hover:text-slate-800 dark:hover:text-white hover:border-amber-400 dark:hover:border-amber-500 transition-all flex items-center justify-center gap-2 hover:bg-amber-50 dark:hover:bg-amber-900/10 active:scale-[0.98]">
