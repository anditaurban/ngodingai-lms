...
export default function VideoWorkspace({ title, setTitle, initialUrl = "", onSave, onDelete }: VideoWorkspaceProps) {
  const [videoUrl, setVideoUrl] = useState(initialUrl);
  const [confirmDelete, setConfirmDelete] = useState(false);
  
  useEffect(() => { setVideoUrl(initialUrl); }, [initialUrl]);
    ...
    <div className="max-w-4xl w-full mx-auto px-6 md:px-16 py-12 pb-40 flex flex-col gap-8 animate-fade-in">
      <div className="flex flex-col gap-4">
        <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800/50 pb-4">
           <div className="flex items-center gap-2 text-blue-500 font-bold text-sm tracking-wider uppercase">
           ...
           <div className="flex items-center gap-3">
              ...
              {/* ✨ MENGIRIM PAYLOAD URL KE ONSAVE ✨ */}
              <button onClick={() => onSave({ url: videoUrl })} className="flex items-center gap-2 px-5 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-xl text-xs font-bold shadow-lg shadow-blue-500/20 transition-all active:scale-95">
                 <span className="material-symbols-outlined text-[16px]">save</span> Simpan Perubahan
              </button>
           </div>
        </div>
        <textarea value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Judul Video..." className={`w-full bg-transparent text-4xl md:text-5xl lg:text-[56px] font-extrabold text-slate-900 dark:text-white placeholder:text-slate-200 dark:placeholder:text-slate-800 resize-none outline-none border-none focus:ring-0 overflow-hidden leading-[1.1] ${googleSansAlt.className}`} rows={2} />
      </div>
      ...
      <div className="bg-white dark:bg-[#111111] p-8 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col gap-6">
         ...
         {videoId ? (
            ...
            <div className="relative w-full aspect-video bg-slate-100 dark:bg-slate-900/50 rounded-2xl border-2 border-dashed border-slate-300 dark:border-slate-700 flex flex-col items-center justify-center text-slate-400 group hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors cursor-pointer">
               ...
               <p className="text-xs mt-1 text-slate-400">Atau klik untuk Upload File MP4 lokal</p>
            </div>
         )}
      </div>
    </div>
